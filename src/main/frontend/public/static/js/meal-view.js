...
function resetMealView() {
    // Add logic to reset the meal view
    // Optionally, make an API call to reset the meal view
    fetch('/api/meal-view/reset', { method: 'POST' })
        .then(() => {
            document.querySelectorAll('.meal-view-table td').forEach(cell => {
                cell.textContent = '';
            });
        })
        .catch(error => console.error('Error resetting meal view:', error));
}
